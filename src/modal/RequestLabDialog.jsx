...
const RequestLabDialog = ({ 
  visible, 
  onClose, 
  onSubmit, 
  onUpdate, // New prop for update
  loading, 
  templates,
  editingTest = null, // Existing test data for editing
  isUpdating = false // Loading state for update
}) => {
  ...
  useEffect(() => {
    if (editingTest && visible) {
      form.setFieldsValue({
        templateId: editingTest.template_id || editingTest.template?.id,
        note: editingTest.notes || ''
      });
      setSelectedTest(editingTest.template_id);
    } else if (visible) {
      form.resetFields();
      setSelectedTest(null);
    }
  }, [editingTest, visible, form]);
  ...
  const handleFinish = (values) => {
    if (editingTest) {
      ...
      onUpdate(editingTest.id, values);
    } else {
      // Create new test
      onSubmit(values);
    }
  ...
  return (
    <Modal
      title={editingTest ? "Update Lab Test" : "Request Lab Test"}
      open={visible}
      ...
      footer={[
        ...
        >
          {editingTest ? "Update Test" : "Request Test"}
        </BhmsButton>,
      ]}
    >
