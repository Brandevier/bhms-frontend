...
// get user details
export const getUserDetails = createAsyncThunk('auth/getUserDetails', async (_, { getState, rejectWithValue }) => {
  ...
  try {
    const response = await apiClient.get(`/auth/user/${user.id}`,);
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(loginUser.fulfilled, (state, action) => {
        ...
        // localStorage.setItem('department_id', state.pendingUser.department);
        console.log('DEPARTMENT ID====', state.pendingUser)
    ...
    builder.addCase(verifyPuzzleAnswer.pending, (state) => {
      ...
      .addCase(verifyPuzzleAnswer.fulfilled, (state, action) => {
        ...
        const deptId = state.user?.department?.id || null;
