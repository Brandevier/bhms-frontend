...
export const addShift = createAsyncThunk(
  ...
  async (shiftData, { rejectWithValue }) => {
    try {
      const response = await apiClient.post("/shifts/create", shiftData);
      return response.data;
    } catch (error) {
...
export const getAllUsersFromDepartment = createAsyncThunk(
  "shifts/getAllUsersFromDepartment",
  async (department_id, { rejectWithValue }) => {
    try {
      const response = await apiClient.get("/shifts/users", { params: { department_id } });
      return response.data;
    } catch (error) {
      return rejectWithValue(error.response?.data || "Failed to fetch users from department");
    }
  }
);
...
const shiftSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(getShifts.pending, (state) => {
      ...
      .addCase(getAllUsersFromDepartment.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(getAllUsersFromDepartment.fulfilled, (state, action) => {
        state.loading = false;
        state.users = action.payload;
      })
      .addCase(getAllUsersFromDepartment.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
      }
      );
