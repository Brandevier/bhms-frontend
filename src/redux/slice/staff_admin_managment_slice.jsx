...
export const getAllStaff = createAsyncThunk(
    ...
    async ({ department_id = null } = {}, { rejectWithValue, getState }) => {
        ...
        try {
            ...
            const response = await apiClient.get("/auth/all-staffs", { params });
...
// assigned primary department to staff
export const assignedPrimaryDepartment = createAsyncThunk(
    "staff/assignedPrimaryDepartment",
    async ({ staff_id, department_id }, { rejectWithValue, getState }) => {
        const { auth } = getState()
        const user = auth.user || auth.admin
        try {
            const response = await apiClient.post(
                `/auth/departments/assign-primary`,
                {
                    staff_id,
                    department_id,
                    institution_id: user.institution.id
...
const staffSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            // assigned primary department to staff
            .addCase(assignedPrimaryDepartment.pending, (state) => {
                state.loading = true;
                state.error = null;
                state.success = false;
            })
            .addCase(assignedPrimaryDepartment.fulfilled, (state, action) => {
                state.loading = false;
                state.success = true;
            })
            .addCase(assignedPrimaryDepartment.rejected, (state, action) => {
                state.loading = false;
                state.error = action.payload;
                state.success = false;
            });
