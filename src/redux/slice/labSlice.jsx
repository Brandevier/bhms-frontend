...
// get lab stats by department
export const getLabTestStats = createAsyncThunk(
  'lab/getLabTestStats',
  async (params = {}, { rejectWithValue }) => {
    try {
      const response = await apiClient.get('/lab/test-stats', { params });
      return response.data;
    } catch (err) {
      return rejectWithValue(err.response?.data || { message: 'Failed to fetch lab test stats' });
    }
  }
);

// get recent lab tests
export const getRecentLabTests = createAsyncThunk(
  'lab/getRecentLabTests',
  async (_, { rejectWithValue }) => {
    try {
      const response = await apiClient.get('/lab/recent-tests');
      return response.data;
    }
    catch (err) {
      return rejectWithValue(err.response?.data || { message: 'Failed to fetch recent lab tests' });
    }
  }
);

// get recents lab test by visit_id
export const getRecentLabTestsByVisitId = createAsyncThunk(
  'lab/getRecentLabTestsByVisitId',
  async (visitId, { rejectWithValue }) => {
    try {
      const response = await apiClient.get(`/lab/recent-tests/visit/${visitId}`);
      return response.data;
    }
    catch (err) {
      return rejectWithValue(err.response?.data || { message: 'Failed to fetch recent lab tests for the visit' });
    }
  }
);
...
const labSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // Lab Test Stats by Department
      .addCase(getLabTestStats.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(getLabTestStats.fulfilled, (state, action) => {
        state.loading = false;
        state.testStats = action.payload.data || [];
      })
      .addCase(getLabTestStats.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload?.message || 'Failed to fetch lab test stats';
      })
      // Recent Lab Tests
      .addCase(getRecentLabTests.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(getRecentLabTests.fulfilled, (state, action) => {
        state.loading = false;
        state.results = action.payload.data || [];
      })
      .addCase(getRecentLabTests.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload?.message || 'Failed to fetch recent lab tests';
      })
      // Recent Lab Tests by Visit ID
      .addCase(getRecentLabTestsByVisitId.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(getRecentLabTestsByVisitId.fulfilled, (state, action) => {
        state.loading = false;
        state.results = action.payload.data || [];
      })
      .addCase(getRecentLabTestsByVisitId.rejected, (state, action) => {
        state.loading = false;
