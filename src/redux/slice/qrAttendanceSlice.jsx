...
export const getLatestQrCode = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      const errorMessage =
        error.response?.data?.message ||  // Most common structure
        error.response?.data?.error ||    // Alternative structure
        error.message ||                  // Native error message
        "Failed to fetch latest QR Code"; // Fallback
...
export const getAttendanceByDepartment = createAsyncThunk(
  "qrAttendance/getAttendanceByDepartment",
  async (departmentId, { rejectWithValue }) => {
    try {
      const response = await apiClient.get(`/attendance/department/${departmentId}`);
      return response.data;
    } catch (error) {
      return rejectWithValue(error.response?.data || "Failed to fetch attendance by department");
    }
  }
);

// get attendance by department and date
export const getAttendanceByDepartmentAndDate = createAsyncThunk(
  "qrAttendance/getAttendanceByDepartmentAndDate",
  async ({ departmentId, date }, { rejectWithValue }) => {
    try {
      const response = await apiClient.get(`/attendance/department/${departmentId}/date/${date}`);
      return response.data;
    } catch (error) {
      return rejectWithValue(error.response?.data || "Failed to fetch attendance by department and date");
    }
  }
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getAttendanceByDepartment.fulfilled, (state, action) => {
        state.loading = false;
        state.attendanceRecords = action.payload.data; // Update attendance records with department filter
      })
      ...
      // Get Attendance by Department and Date
      .addCase(getAttendanceByDepartmentAndDate.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(getAttendanceByDepartmentAndDate.fulfilled, (state, action) => {
        state.loading = false;
        state.attendanceRecords = action.payload.data; // Update attendance records with department and date filter
      })
      .addCase(getAttendanceByDepartmentAndDate.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
      });
    ;
