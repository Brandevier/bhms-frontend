...
import { Card, Typography, Radio, message, Spin } from "antd";
...
const PuzzleAuthentication = () => {
    ...
    const handleSubmit = () => {
        ...
        dispatch(verifyPuzzleAnswer({ staffID: pendingUser.staffID, selectedAnswer: selectedAnswer }))
            ...
            .catch((err) => {
                message.error(err.message || "Incorrect answer. Please try again.");
            })
            .finally(() => {
                setSubmitting(false);
            });
            ...
            <Card style={{ width: 500, padding: "20px", borderRadius: "10px", textAlign: "center", boxShadow: "0 4px 10px rgba(0,0,0,0.1)" }}>
                ...
                <div style={{ marginTop: "20px", textAlign: "left" }}>
                    ...
                    <Radio.Group 
                        ...
                        style={{ width: "100%" }}
                        disabled={submitting} // Disable radio group during submission
                    >
                        {pendingUser.options.map((option, index) => (
                            <Radio 
                                key={index} 
                                value={option} 
                                style={{ display: "block", padding: "8px", fontSize: "16px" }}
                            >
                                {option}
                            </Radio>
                        ))}
                ...
                <BhmsButton
                    ...
                    loading={submitting}
                    disabled={!selectedAnswer || submitting} // Disable if no answer selected or submitting
                    className="mt-5"
                >
                ...
                {/* Error Message */}
                {error && <Text type="danger" style={{ display: "block", marginTop: "10px" }}>{error.message}</Text>}
            </Card>
        </div>
    );
};
