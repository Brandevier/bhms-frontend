...
import { Card, List, Avatar, Button, Tooltip, Empty, Input, message, Collapse, Row } from "antd";
...
const PatientNotes = ({ patient_notes, general_handler, patient_id }) => {
    ...
    const handleSaveNote = (note) => {
        dispatch(createPatientNote(note)).unwrap()
            .then(() => {
    ...
    const handleAddComment = (noteId) => {
        ...
        // Handle comment submission logic here (e.g., dispatch action)
                    ...
                    renderItem={(note) => (
                        <Card style={{ marginBottom: 10, padding: 10 }} bordered>
                            ...
                            <Collapse style={{ marginTop: 10 }}>
                                <Panel header={<><CommentOutlined /> Contribute</>} key={note.id}>
                                   <div className="flex">
                                   <Input.TextArea
                                        value={commentText}
